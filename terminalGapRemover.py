	...
	for s in seqs.keys():
		...
		for nuc in new:
			if nuc == "-":
				...
				break
		if len(left_ns) != len(new):
			for nuc in reversed(new):
				if nuc == "-":
					right_ns = right_ns + "N"
				else:
					break
		if len(left_ns) > 0:
			new = left_ns + new[len(left_ns):]
		if len(right_ns) > 0:
			new = new[0:len(new)-len(right_ns)] + right_ns
		seqs[s] = new
...
class parseArgs():
	def __init__(self):
		...
		if self.fasta:
			self.out = os.path.splitext(self.fasta)[0] + '.gapfix.fasta'
		elif self.phylip:
			self.out = os.path.splitext(self.phylip)[0] + '.gapfix.phylip'
	...
	def display_help(self, message=None):
		...
		print ("Contact:Tyler K. Chafin")
		print ("\nUsage: ", sys.argv[0], "[-f <.fasta>] [-p <.phy>]\n")
		print ("Description: Simple script to convert terminal gap characters to N's. Accepts FASTA or PHYLIP")

		print("""
	Arguments:
		-f,--fasta	: Input FASTA to be converted
		-p,--phy	: Input PHYLIP to be converted
